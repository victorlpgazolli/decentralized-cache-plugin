...
private class ManifestCacheHelperImpl(
    ...
    override fun fetchCacheEntries(): Map<String, String> {
        ...
        val merged = peers.take(2).fold(emptyMap<String, String>()) { acc, peerIpns ->
            val remoteManifest = fetchManifestFromPeer(from = peerIpns)

            return@fold acc + remoteManifest
        }
        logger.log("fetchCacheEntries", "Fetched cache entries from peers (${peers.size}): ${merged.keys}")
        return merged
    }
    ...
    override fun writeToManifest(cacheEntries: Map<String, String>): CacheKeyType.FilePath? {
        ...
        val newManifest = CacheManifestResult.Success(
            publishKeyName = manifest?.publishKeyName ?: "self",
            hashs = cacheEntries
        )

        val manifestFile = File("/tmp/manifest.json").apply {
            createNewFile()
        ...
        val result = localCacheProvider.put(
    ...
    fun fetchManifestFromPeer(from: String): Map<String, String> {
        val manifestContent = runCatching {
            val manifestIpfsPath = ipfsConnectedSession
                .nameResolve<ManifestFromPeer>(from)
                .path

            ipfsConnectedSession.catText(manifestIpfsPath)
        }.onFailure {
            logger.log("fetchManifest", "Peer $from failed to provide a manifest (IPNS unresolvable or timeout): ${it.message}")
        }.getOrDefault("")
    ...
    private fun String.decodeManifest(): Map<String, String> {
        ...
        runCatching {
            ...
            return when(val manifestResult = json.decodeFromString<CacheManifestResult>(this)) {
            ...
            logger.log("decodeManifest", "failed to decode manifest $this; cause -> ${it.message}")
        }.onSuccess {
            logger.log("decodeManifest", "decoded manifest: $it")
        }
        return emptyMap()
    }
