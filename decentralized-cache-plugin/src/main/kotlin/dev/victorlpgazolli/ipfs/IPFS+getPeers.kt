...
@Serializable
data class SwarmPeersResponse(@SerialName("Peers") val peers: List<Peer>? = emptyList())

@Serializable data class Peer(@SerialName("Peer") val id: String)

fun IpfsConnectedSession.getPeers(): List<String> =
    runCatching {
        ...
        .onFailure { println("[IpfsConnectedSession] Error to fetch peers: ${it.message}") }
        .getOrNull() ?: emptyList()
