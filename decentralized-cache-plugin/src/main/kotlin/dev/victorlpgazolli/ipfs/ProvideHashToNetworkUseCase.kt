...
import dev.victorlpgazolli.cache.CacheManifestResult
...
internal class ProvideHashToNetworkUseCaseImpl(
    ...
    override fun invoke(cacheKey: String, ipfsHash: String) {
        runCatching {
            ...
            ipfsConnectedSession.connection.callCmd("routing/provide?arg=$ipfsHash").use(ResponseBody::string)

            CompletableFuture.runAsync {
                updateAndPublishManifest(cacheKey, ipfsHash)
            }
    ...
    private fun updateAndPublishManifest(cacheKey: String, ipfsHash: String) = runCatching {
        logger.log("ManifestUpdate", "Publishing manifest $ipfsHash to IPNS...")
        val publishResponse = ipfsConnectedSession.connection
